...
void DailyResetIfNeeded()
{
    ...
    if(TimeCurrent(currentTime) != TimeCurrent(day_marker)){
...
void EvaluateAndTrade()
{
    ...
    double h1_sma50 = iMA(_Symbol, PERIOD_H1, 50, 0, MODE_SMA, PRICE_CLOSE, 1);
    ...
    for(int s=0;s<sig_count;s++){
      ...
      if(CanTradeToday()){
        ...
        double atr = iATR(_Symbol, pa_tf, ATR_Period, 1); if(atr<=0) atr = iATR(_Symbol, PERIOD_M15, ATR_Period, 1);
    ...
    if(hasPos){
      ...
      double atrm = iATR(_Symbol, PERIOD_M15, ATR_Period, 1);
