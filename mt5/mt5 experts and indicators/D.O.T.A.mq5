...
input bool   UseAILearning        = true;    // Enable AI pattern learning
input int    MinTradesForLearning = 20;      // Minimum trades before strict filtering
input double MinPatternScore      = 0.6;     // Minimum score to take trade (0-1)
input int    PatternMemory        = 1000;    // Maximum number of trades to remember
input bool   UseMarketContext     = true;    // Consider market conditions in scoring
...
input string api_key              = "sk-secure-api-key-2025-xyz-abc123def456";
...
struct PatternStats {
    ...
    double expectancy;
    double confidence;
    double vol_low_winrate;
    double vol_high_winrate;
    double london_winrate;
    double ny_winrate;
...
struct DeltaWindow {
    ...
    int count;
    void Clear(){ head=0; count=0; for(int i=0;i<8;i++){ buf[i].delta=0; buf[i].cum_delta=0; buf[i].max_delta=0; buf[i].min_delta=0; buf[i].t=0; } }
    void Init(){ Clear(); }
...
PatternStats g_pattern_performance[];
string g_patterns[];
...
void InitializeAIPatterns()
{
     string patterns[] = {
         "Breakout_Delta_Surge",
         "Breakout_Delta_Transition",
         "Breakout_Delta_Flip",
         "Retest_Delta_Surge",
         "Retest_Delta_Transition",
         "Retest_Delta_Flip",
         "Engulfing_Delta_Surge",
         "Engulfing_Delta_Transition",
         "Engulfing_Delta_Flip",
         "Pin_Delta_Surge",
         "Pin_Delta_Transition",
         "Pin_Delta_Flip",
         "Inside_Delta_Surge",
         "Inside_Delta_Transition",
         "Inside_Delta_Flip"
     };

     g_pattern_count = ArraySize(patterns);
     ArrayResize(g_pattern_performance, g_pattern_count);
     ArrayResize(g_patterns, g_pattern_count);

     for(int i=0; i < g_pattern_count; i++) {
         g_patterns[i] = patterns[i];
         g_pattern_performance[i].total_trades = 0;
         g_pattern_performance[i].winning_trades = 0;
         g_pattern_performance[i].confidence = 0;
         g_pattern_performance[i].vol_low_winrate = 0;
         g_pattern_performance[i].vol_high_winrate = 0;
         g_pattern_performance[i].london_winrate = 0;
         g_pattern_performance[i].ny_winrate = 0;
     }
}
...
MarketContext GetMarketContext()
{
     ...
     ctx.volatility = iATR(_Symbol, PERIOD_H1, 14) /
                     iMA(_Symbol, PERIOD_H1, 14, 0, MODE_SMA, iATR(_Symbol, PERIOD_H1, 14));

     // Calculate trend strength using EMA slope difference (alternative to ADX MODE_MAIN)
     // This measures the absolute difference between fast and slow EMA as a proxy for trend momentum
     double fastEMA = iMA(_Symbol, PERIOD_H1, 5, 0, MODE_EMA, PRICE_CLOSE);
     double slowEMA = iMA(_Symbol, PERIOD_H1, 10, 0, MODE_EMA, PRICE_CLOSE);
     if (fastEMA <= 0.0 || slowEMA <= 0.0) {
         ctx.trend_strength = 0.0;
     } else {
         ctx.trend_strength = MathAbs(fastEMA - slowEMA) * 1000; // Scale for comparability
     }

     MqlDateTime dt;
     TimeCurrent(dt);
     ctx.market_hours = dt.hour;

     double spread_sum = 0;
     for(int i=0; i<10; i++) {
         spread_sum += SymbolInfoInteger(_Symbol, SYMBOL_SPREAD);
     }
     ctx.avg_spread = spread_sum / 10.0;

     return ctx;
}

// Calculate pattern score with market context
double GetPatternScore(string pattern, MarketContext &ctx)
{
     for(int i=0; i < g_pattern_count; i++) {
         if(g_patterns[i] == pattern) {
             PatternStats stats = g_pattern_performance[i];

             // Base scoring
             double win_rate_score = stats.win_rate * 0.3;
             double expectancy_score = MathMin(1.0, stats.expectancy / 20.0) * 0.3;
             double confidence_score = stats.confidence * 0.2;
             ...
             if(UseMarketContext) {
                 ...
                 if(ctx.volatility < 1.2) {
                     context_score += stats.vol_low_winrate * 0.1;
                 } else {
                     context_score += stats.vol_high_winrate * 0.1;
                 }

                 // Session adjustment
                 bool isLondon = (ctx.market_hours >= 8 && ctx.market_hours <= 16);
                 bool isNY = (ctx.market_hours >= 13 && ctx.market_hours <= 21);
                 if(isLondon) context_score += stats.london_winrate * 0.05;
                 if(isNY) context_score += stats.ny_winrate * 0.05;

                 // Spread penalty
                 double spread_factor = (ctx.avg_spread > 20) ? 0.7 : 1.0;

                 // Trend alignment bonus
                 double trend_bonus = (ctx.trend_strength > 25) ? 0.1 : 0;

                 return MathMin((win_rate_score + expectancy_score + confidence_score + context_score) *
                              spread_factor + trend_bonus, 1.0);
             }

             return MathMin(win_rate_score + expectancy_score + confidence_score, 1.0);
         }
     }
     return 0;
}

// Record and update trade statistics
void RecordTrade(string pattern, bool isWin, double profit, double entry_price,
                 double exit_price, int direction, double delta, double atr)
{
     MarketContext ctx = GetMarketContext();

     // Manage trade history array
     int size = ArraySize(g_trade_history);
     if(size >= PatternMemory) {
         for(int i=0; i<size-1; i++) {
             g_trade_history[i] = g_trade_history[i+1];
         }
         size--;
     }
     ...
     // Record new trade
     g_trade_history[size].entry_time = TimeCurrent();
     g_trade_history[size].direction = direction;
     g_trade_history[size].entry_price = entry_price;
     g_trade_history[size].exit_price = exit_price;
     g_trade_history[size].profit = profit;
     g_trade_history[size].pattern = pattern;
     g_trade_history[size].delta_value = delta;
     g_trade_history[size].atr_value = atr;
     g_trade_history[size].market_volatility = ctx.volatility;
     g_trade_history[size].market_hour = ctx.market_hours;
     g_trade_history[size].success = isWin;

     // Update pattern statistics
     UpdatePatternStats(pattern, isWin, profit, ctx);
}

// Update pattern statistics
void UpdatePatternStats(string pattern, bool isWin, double profit, MarketContext &ctx)
{
     for(int i=0; i < g_pattern_count; i++) {
         if(g_patterns[i] == pattern) {
             PatternStats stats = g_pattern_performance[i];
             ...
             if(isWin) {
                 ...
                 stats.last_success = TimeCurrent();
             } else {
                 stats.last_failure = TimeCurrent();
             }

             stats.win_rate = (double)stats.winning_trades / stats.total_trades;
             stats.confidence = MathMin(1.0, stats.total_trades / 50.0);

             if(ctx.volatility < 1.2) {
                 stats.vol_low_winrate = (stats.vol_low_winrate * 0.95) + (isWin ? 0.05 : 0);
             } else {
                 stats.vol_high_winrate = (stats.vol_high_winrate * 0.95) + (isWin ? 0.05 : 0);
             }

             if(ctx.market_hours >= 8 && ctx.market_hours <= 16) {
                 stats.london_winrate = (stats.london_winrate * 0.95) + (isWin ? 0.05 : 0);
             }
             if(ctx.market_hours >= 13 && ctx.market_hours <= 21) {
                 stats.ny_winrate = (stats.ny_winrate * 0.95) + (isWin ? 0.05 : 0);
             }

             // Update expectancy
             CalcuatePatternExpectancy(i);
             break;
         }
     }
}
...
void CalcuatePatternExpectancy(int pattern_index)
{
     double total_profit = 0, total_loss = 0;
     int wins = 0, losses = 0;

     for(int j=0; j < ArraySize(g_trade_history); j++) {
         if(g_trade_history[j].pattern == g_patterns[pattern_index]) {
             if(g_trade_history[j].profit > 0) {
                 total_profit += g_trade_history[j].profit;
                 wins++;
             } else {
                 total_loss += MathAbs(g_trade_history[j].profit);
                 losses++;
             }
         }
     }

     PatternStats stats = g_pattern_performance[pattern_index];
     stats.avg_profit = (wins > 0) ? total_profit/wins : 0;
     stats.avg_loss = (losses > 0) ? total_loss/losses : 0;
     stats.expectancy = (stats.win_rate * stats.avg_profit) -
...
void DailyResetIfNeeded()
{
    ...
    if(currentTime != day_marker){
      ...
      Print(StringFormat("New trading day started. Start equity=%.2f", day_start_equity));
    }
}
bool CanTradeToday()
{
    ...
    if(loss_pct >= MaxDailyLossPercent){
      Print(StringFormat("Daily loss cap reached: %.2f%% - pausing entries.", loss_pct));
      return false;
    }
    return true;
}
...
void EvaluateAndTrade()
{
    ...
    for(int s=0;s<sig_count;s++){
      ...
      if(confirm_votes >= needed_votes && CanTradeToday()){
        string pattern = ps.reason;
        if(UseDeltaFlip) pattern += "_Delta_Flip";
        else if(UseDeltaSurge) pattern += "_Delta_Surge";
        else if(UseDeltaTransition) pattern += "_Delta_Transition";
        ...
        if(UseAILearning) {
          ctx = GetMarketContext();
          pattern_score = GetPatternScore(pattern, ctx);

          if(g_pattern_performance[0].total_trades >= MinTradesForLearning) {
            should_trade = (pattern_score >= MinPatternScore);
          }
        }
        ...
        if(should_trade) {
          ...
          if(desired_dir == 1) { // buy
            ...
            if(trade_ok) {
              Print("ENTER BUY | PA=", ps.tf, " reason=", ps.reason, " lot=", DoubleToString(lot,2));
              if(UseAILearning) {
                Print("Trade executed | Pattern: ", pattern,
                      " | Score: ", DoubleToString(pattern_score,2),
                      " | Market vol: ", DoubleToString(ctx.volatility,2),
                      " | Trend: ", DoubleToString(ctx.trend_strength,2));
              }
            } else {
            ...
            if(trade_ok) {
              Print("ENTER SELL | PA=", ps.tf, " reason=", ps.reason, " lot=", DoubleToString(lot,2));
              if(UseAILearning) {
                Print("Trade executed | Pattern: ", pattern,
                      " | Score: ", DoubleToString(pattern_score,2),
                      " | Market vol: ", DoubleToString(ctx.volatility,2),
          ...
          Print("Trade rejected by AI - Score too low: ",
                DoubleToString(pattern_score,2), " | Pattern: ", pattern);
        }
      } // if confirmed
    } // for each PA signal
    ...
    if(hasPos){
      ...
      if(flips >= req && profit > 0){
        ...
        if(close_ok && UseAILearning) {
          // Record the trade result
          double entry_price = PositionGetDouble(POSITION_PRICE_OPEN);
          double exit_price = (pos_type == 1) ? SymbolInfoDouble(_Symbol, SYMBOL_BID) : SymbolInfoDouble(_Symbol, SYMBOL_ASK);
          string pattern = ""; // Need to track pattern from entry
          RecordTrade(pattern, profit > 0, profit, entry_price, exit_price, pos_type, 0, 0); // Delta and ATR not available here
        }
