...
class DashboardApp:
    ...
    STALE_SECS  = 10        # treat JSON older than this as OFFLINE
    ...
    def _build_panels(self):
        ...
        self._warn_frame.pack(fill="both", expand=True, padx=6, pady=4)
        self._warn_labels: list[tk.Label] = []

        # ── Panel 8 — Pipeline Log ────────────────────────────────────────────
        p8 = FloatingPanel(C, "Pipeline Log", 380, 500, 380, top_color=ACCENT)
        log_outer = tk.Frame(p8.body, bg=PANEL)
        log_outer.pack(fill="both", expand=True, padx=6, pady=4)
        self._log_text = tk.Text(
            log_outer, bg=BORDER, fg=ACCENT,
        ...
        self._panel_names = [
            ...
            "Account","Positions","Warnings","Pipeline"
        ]
